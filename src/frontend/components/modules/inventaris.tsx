...
function PengolahanTab() {
  ...
  const openCreate = async () => {
    ...
    setDialogOpen(true)
    try {
      const [w, p, inv] = await Promise.all([api.barang.list(), api.produk.list(), api.inventaris.stok(), loadRecipes()])
      setWasteItems(w as WasteItemRow[])
      setProducts(p as ProductRow[])
  ...
  const updateOutput = (idx: number, field: keyof OutputRow, val: string) => {
    ...
    // ============================================================
    // AUTO-FILL bahan baku dari resep saat produk dipilih
    // ------------------------------------------------------------
    ...
    if (field === 'productId' && val) {
      ...
      if (productRecipes.length > 0) {
        const outQty = parseFloat(outputs[idx]?.quantity || '0') || 1
        const newInputs = productRecipes.map((r) => ({
          wasteItemId: r.wasteItemId,
          source: r.source || 'nabung',
          quantity: String((r.quantityPerUnit * outQty).toFixed(2)),
        }))
        setInputs(newInputs)
        toast.info(`Resep aktif: ${productRecipes.length} bahan baku otomatis terisi`, {
          description: productRecipes
            .map((r) => `${r.wasteItemName}: ${r.quantityPerUnit} ${r.wasteItemUnit}/unit`)
            .join(', '),
        })
      }
    }
    ...
    if (field === 'quantity' && outputs[idx]?.productId) {
      ...
      if (productRecipes.length > 0) {
        const outQty = parseFloat(val) || 0
        setInputs((prev) =>
          prev.map((inp, i) => {
            if (i < productRecipes.length) {
              const recipe = productRecipes[i]
              return {
                ...inp,
                wasteItemId: recipe.wasteItemId,
                source: recipe.source,
                quantity: String((recipe.quantityPerUnit * outQty).toFixed(2)),
              }
            }
            return inp
          })
      ...
      <CardHeader className="border-b border-emerald-100/70">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              onClick={() => { loadRecipes(); setRecipeOpen(true) }}
...
function RecipeSettingsDialog({
  ...
  recipes: RecipeRow[]
  products: ProductRow[]
  wasteItems: WasteItemRow[]
  onRefresh: () => void
}) {
  const [saving, setSaving] = React.useState(false)
  // Form state
  const [fProductId, setFProductId] = React.useState('')
  const [fWasteItemId, setFWasteItemId] = React.useState('')
  const [fQtyPerUnit, setFQtyPerUnit] = React.useState('')
  const [fSource, setFSource] = React.useState('nabung')
  const [fNotes, setFNotes] = React.useState('')

  const submit = async () => {
    if (!fProductId || !fWasteItemId || !fQtyPerUnit) {
      toast.error('Produk, bahan baku, dan jumlah per unit wajib diisi')
      return
    }
    setSaving(true)
    try {
      const res = await fetch('/api/inventaris/resep', {
        ...
        body: JSON.stringify({
          ...
          wasteItemId: fWasteItemId,
          quantityPerUnit: parseFloat(fQtyPerUnit),
          source: fSource,
          isActive: true,
      ...
      toast.success('Resep disimpan', {
        description: `${products.find((p) => p.id === fProductId)?.name} = ${fQtyPerUnit} ${wasteItems.find((w) => w.id === fWasteItemId)?.unit || 'kg'} ${wasteItems.find((w) => w.id === fWasteItemId)?.name}`,
      })
      setFProductId(''); setFWasteItemId(''); setFQtyPerUnit(''); setFNotes('')
      onRefresh()
    } catch (e: any) {
  ...
  const toggleActive = async (id: string, isActive: boolean) => {
    try {
      ...
      onRefresh()
      toast.success(isActive ? 'Resep dinonaktifkan (mode manual)' : 'Resep diaktifkan (auto-fill)')
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-emerald-900">
            <Layers className="h-5 w-5 text-emerald-600" /> Pengaturan Resep Pengolahan
          </DialogTitle>
          <DialogDescription>
            Atur bahan baku otomatis per produk. Jika aktif, bahan baku akan terisi otomatis saat pilih produk.
            Jika tidak aktif, admin input manual.
          </DialogDescription>
        </DialogHeader>

        {/* Form tambah resep */}
        <div className="rounded-lg border border-emerald-100 bg-emerald-50/30 p-4">
          <p className="mb-3 text-sm font-semibold text-emerald-800">Tambah / Update Resep</p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs text-emerald-700/70">Produk Hasil</Label>
              <Select value={fProductId} onValueChange={setFProductId}>
                <SelectTrigger className="w-full bg-white"><SelectValue placeholder="Pilih produk..." /></SelectTrigger>
                <SelectContent>
                  {products.map((p) => (
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-emerald-700/70">Bahan Baku</Label>
              <Select value={fWasteItemId} onValueChange={setFWasteItemId}>
                <SelectTrigger className="w-full bg-white"><SelectValue placeholder="Pilih bahan baku..." /></SelectTrigger>
                <SelectContent>
                  {wasteItems.map((w) => (
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-emerald-700/70">Jumlah Bahan per 1 Unit Produk</Label>
              <Input
                type="number"
                inputMode="decimal"
                min="0"
                step="0.01"
                placeholder="Contoh: 1.67 (kg per pcs)"
                value={fQtyPerUnit}
                onChange={(e) => setFQtyPerUnit(e.target.value)}
                className="bg-white"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Select value={fSource} onValueChange={setFSource}>
                ...
                <SelectContent>
                  <SelectItem value="nabung">Nabung (dari nasabah)</SelectItem>
                  <SelectItem value="sedekah">Sedekah (donasi)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          <div className="mt-2 space-y-1.5">
            <Label className="text-xs text-emerald-700/70">Catatan (opsional)</Label>
            <Input
            ...
            <div className="space-y-2">
              {recipes.map((r) => (
                <div key={r.id} className={cn(
                  ...
                  r.isActive ? 'border-emerald-200 bg-emerald-50/40' : 'border-gray-200 bg-gray-50/40'
                )}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-emerald-900">{r.productName}</span>
                        <Badge className={r.isActive
                          ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-100'
                          : 'bg-gray-200 text-gray-600 hover:bg-gray-200'
                        }>
                          {r.isActive ? 'Aktif (auto)' : 'Nonaktif (manual)'}
                        </Badge>
                      </div>
                      <p className="mt-0.5 text-xs text-emerald-700/80">
                        = <strong>{r.quantityPerUnit} {r.wasteItemUnit}</strong> {r.wasteItemName} ({r.wasteItemCode})
                        <span className="text-emerald-600/60"> · sumber: {r.source === 'sedekah' ? 'Sedekah' : 'Nabung'}</span>
                      </p>
                      {r.notes && <p className="mt-0.5 text-xs italic text-emerald-600/60">{r.notes}</p>}
                    </div>
