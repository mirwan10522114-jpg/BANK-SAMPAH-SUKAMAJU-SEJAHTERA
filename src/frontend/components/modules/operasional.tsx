...
function Riwayat() {
  ...
  const buildRows = (
    ...
    // Sort nabung by date ascending for sequence numbering
    const sortedNabung = [...nabung].sort((a, b) => new Date(a.transactedAt).getTime() - new Date(b.transactedAt).getTime())
    const nabungDateCount: Record<string, number> = {}
    const nabungRows: RiwayatRow[] = sortedNabung.map((t) => {
      ...
      nabungDateCount[dateKey] = (nabungDateCount[dateKey] || 0) + 1
    ...
    // Sort sedekah by date ascending for sequence numbering
    const sortedSedekah = [...sedekah].sort((a, b) => new Date(a.transactedAt || a.filterAt).getTime() - new Date(b.transactedAt || b.filterAt).getTime())
    const sedekahDateCount: Record<string, number> = {}
    const sedekahRows: RiwayatRow[] = sortedSedekah.map((t) => {
      const d = new Date(t.transactedAt || t.filterAt)
      const dateKey = `${String(d.getDate()).padStart(2,'0')}${String(d.getMonth()+1).padStart(2,'0')}${d.getFullYear()}`
      sedekahDateCount[dateKey] = (sedekahDateCount[dateKey] || 0) + 1
            ...
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-emerald-50">
                <TableRow className="border-emerald-200">
                  ...
                  <TableHead className="text-emerald-800">Kode Transaksi</TableHead>
              ...
              <TableBody>
                {filtered.map((r) => (
                  <TableRow key={`${r.jenis}-${r.id}`} className="border-emerald-100">
                    ...
                    <TableCell>
                      <span className="rounded bg-emerald-50 px-1.5 py-0.5 font-mono text-[10px] font-bold text-emerald-700">{r.kodeTransaksi || '-'}</span>
                    </TableCell>
                    <TableCell>
                      {r.jenis === 'nabung' ? (
                        <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200 hover:bg-emerald-100">
...
function QcQueueView() {
  ...
  const [editItems, setEditItems] = useState<Record<string, { quantityAfterQc: number; qcReason: string }>>({})
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/operasional/qc-queue')
      const data = await res.json()
      setQueue(data.queue || [])
    } catch (e: any) {
      toast.error('Gagal memuat antrian QC: ' + e.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSelect = (tx: any) => {
    setSelected(tx)
    const init: Record<string, { quantityAfterQc: number; qcReason: string }> = {}
    for (const item of tx.items || []) {
      init[item.id] = {
        quantityAfterQc: toNumber(item.quantityBeforeQc), // default: sama dengan berat kotor
        qcReason: '',
      }
    }
    setEditItems(init)
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const editItemsArr = Object.entries(editItems).map(([id, v]) => ({
        id,
        quantityAfterQc: v.quantityAfterQc,
          ...
          {queue.length === 0 ? (
            ...
            <div className="space-y-2">
              {queue.map((tx) => (
                <div
                  key={tx.id}
                  className="flex items-center justify-between rounded-lg border border-amber-200 bg-white p-3 hover:border-amber-400 cursor-pointer transition-colors"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input
                      ...
                      step="0.001"
                      min="0"
                      value={editItems[item.id]?.quantityAfterQc ?? 0}
                      onChange={(e) => setEditItems((prev) => ({
                        ...prev,
                        [item.id]: {
                          quantityAfterQc: parseFloat(e.target.value) || 0,
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setEditItems((prev) => ({
                        ...prev,
                        [item.id]: {
                          quantityAfterQc: prev[item.id]?.quantityAfterQc ?? 0,
            ...
            <div className="rounded-md border border-rose-200 bg-rose-50/30 p-3 space-y-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  id="rejectAll"
                  checked={rejectMode}
                  onChange={(e) => setRejectMode(e.target.checked)}
