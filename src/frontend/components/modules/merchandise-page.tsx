  ...
  return (
    ...
    >
      ...
      <CardContent className="p-3 sm:p-4">
        ...
        <p className="mb-1 text-sm font-extrabold text-[#0d9488] sm:text-base">
          {formatRupiah(product.price)}
        </p>
        {product.dijualDenganPoin && product.pointsCost && product.pointsCost > 0 ? (
          <p className="mb-2 text-xs font-semibold text-violet-600">
            🎁 atau {product.pointsCost} poin
          </p>
        ) : (
          <div className="mb-2" />
        )}
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-4 sm:px-6 sm:py-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:gap-10">
        ...
        <div className="flex flex-col">
          ...
          <p className="mb-2 text-2xl font-extrabold text-[#0d9488] sm:text-3xl">
            {formatRupiah(product.price)}
          </p>
          {product.dijualDenganPoin && product.pointsCost && product.pointsCost > 0 ? (
            <p className="mb-4 text-sm font-bold text-violet-600 bg-violet-50 inline-block px-3 py-1 rounded-full">
              🎁 atau tukar dengan {product.pointsCost} poin
            </p>
          ) : (
            <div className="mb-4" />
          )}
...
function CheckoutView({
  ...
  const [courierOptions, setCourierOptions] = React.useState<
    { serviceDisplay: string; cost: number; etd: string; kurir?: string; service?: string; courierName?: string; description?: string }[]
  >([])
  // Track courier yang dipilih user (index di courierOptions, -1 = belum pilih)
  const [selectedCourierIdx, setSelectedCourierIdx] = React.useState<number>(-1)
    ...
    fetch('/api/shipping/cost', {
      ...
      .then((data: any) => {
        ...
        const mapped = opts.map((o: any) => ({
          serviceDisplay: o.serviceDisplay || `${o.kurir || ''} ${o.service || ''}`.trim(),
        ...
        setCourierOptions(mapped)
        setOngkirSource(data.source || 'fallback')
        setOngkirMessage(data.message || '')
        // Reset pilihan courier → paksa user pilih sendiri
        setSelectedCourierIdx(-1)
        setOngkir(0)
        // Kalau hanya ada 1 opsi (atau fallback tanpa opsi), auto-pilih
        if (mapped.length === 1) {
          setSelectedCourierIdx(0)
          setOngkir(mapped[0].cost)
        } else if (mapped.length === 0) {
          // Fallback tanpa opsi kurir → pakai cheapestCost
  ...
  // Handler saat user klik salah satu opsi kurir
  const handleSelectCourier = (idx: number) => {
    setSelectedCourierIdx(idx)
    setOngkir(courierOptions[idx]?.cost || 0)
  }
  ...
  const handleSubmit = async () => {
    ...
    // Validasi: kalau ada opsi kurir, wajib pilih salah satu
    if (courierOptions.length > 0 && selectedCourierIdx === -1) {
      toast.error('Silakan pilih kurir/ongkir yang diinginkan')
      return
    }
    // Ambil kurir yang dipilih user
    ...
    const formHash = JSON.stringify({
      ...
      kurir: selectedCourier ? `${selectedCourier.kurir}-${selectedCourier.service}` : 'default',
      ongkir: selectedCourier ? selectedCourier.cost : ongkir,
    })
      ...
      if (isCacheFresh && cached) {
        ...
        const createRes = await fetch('/api/payment/create', {
          ...
          body: JSON.stringify({
            ...
            // Kirim kurir & ongkir yang dipilih user
            courier: selectedCourier
              ? {
                  kurir: selectedCourier.kurir || '',
                  service: selectedCourier.service || '',
                  serviceDisplay: selectedCourier.serviceDisplay || '',
                  courierName: selectedCourier.courierName || '',
                  description: selectedCourier.description || '',
                  etd: selectedCourier.etd || '',
                  cost: selectedCourier.cost,
                }
              : undefined,
            ongkirOverride: selectedCourier ? selectedCourier.cost : undefined,
          }),
        })
                ...
                {courierOptions.length > 0 && (
                  <div className="rounded-md border border-emerald-900/10 bg-emerald-50/30 p-2">
                    <p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-700">
                      Pilih Kurir (wajib pilih salah satu)
                    </p>
                    <div className="max-h-48 space-y-1 overflow-y-auto text-[11px]">
                      {courierOptions.map((o, idx) => (
                        <label
                          key={`${o.serviceDisplay}-${idx}`}
                          className={`flex cursor-pointer items-start justify-between gap-2 rounded border p-2 transition-colors ${
                            selectedCourierIdx === idx
                              ? 'border-emerald-500 bg-emerald-100/70 ring-1 ring-emerald-400'
                        ...
                        >
                          <div className="flex items-start gap-2">
                            <input
                              ...
                              checked={selectedCourierIdx === idx}
                              onChange={() => handleSelectCourier(idx)}
                            />
                            <div className="flex-1">
                              ...
                              <div className="text-[10px] text-emerald-700/70">
                                {o.courierName ? `${o.courierName} · ` : ''}ETD {o.etd}
                                {o.description ? ` · ${o.description}` : ''}
                              </div>
                            </div>
                          </div>
                          <span className="whitespace-nowrap font-bold text-emerald-900">
                            {formatRupiah(o.cost)}
                          </span>
                        </label>
                      ))}
                    </div>
                    {selectedCourierIdx === -1 && (
                      <p className="mt-1 text-[10px] italic text-rose-600">
                        ⚠ Pilih salah satu kurir di atas untuk melanjutkan
                      </p>
                    )}
                  </div>
                )}
