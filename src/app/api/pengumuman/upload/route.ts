...
// POST: Upload gambar pengumuman ke /public/uploads/pengumuman/
export async function POST(req: NextRequest) {
  ...
  if (!file || !(file instanceof File)) {
    return NextResponse.json(
      { success: false, error: 'File tidak ditemukan pada request' },
      { status: 400 }
    )
  }
  ...
  const filename = `pengumuman-${timestamp}-${rand}.${ext}`
  ...
  const url = `/uploads/pengumuman/${filename}`
