...
// outputs bisa berupa productId (produk yang sudah ada) ATAU newProduct (buat produk baru otomatis)
export async function POST(req: NextRequest) {
  ...
  const { inputs, outputs, notes } = body as {
    ...
    outputs: {
      productId?: string          // ID produk yang sudah ada
      quantity: number
      // Field untuk auto-create produk baru (jika productId kosong):
      newProductName?: string
      newProductPrice?: number
      newProductUnit?: string
      newProductWeightGram?: number
      newProductCategory?: string  // ID kategori produk (opsional)
    }[]
    notes?: string
  }
