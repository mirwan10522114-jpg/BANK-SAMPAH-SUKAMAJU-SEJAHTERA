...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (tx.partner?.email) {
      ...
      html += `<div class="struk-section"><div class="info-row"><span class="key">No. Invoice</span><span class="val mono">${invoiceNumber}</span></div><div class="info-row"><span class="key">Tanggal</span><span class="val">${new Date().toLocaleString('id-ID')}</span></div><div class="info-row"><span class="key">Mitra</span><span class="val bold">${tx.partner.name}</span></div></div>`
