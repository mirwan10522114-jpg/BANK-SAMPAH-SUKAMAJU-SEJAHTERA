...
export async function GET(req: NextRequest) {
  ...
  // Sort by transactedAt ascending to assign sequential numbers
  const sortedSavings = [...savingLogs].sort((a, b) => new Date(a.transactedAt).getTime() - new Date(b.transactedAt).getTime())
  const sortedSedekahs = [...sedekahLogs].sort((a, b) => new Date(a.transactedAt).getTime() - new Date(b.transactedAt).getTime())

  // Counters per date for sequence
  const nabungDateCount: Record<string, number> = {}
  const sedekahDateCount: Record<string, number> = {}

  const logRows: LogRow[] = []
  for (const t of sortedSavings) {
    const dateKey = ddMMYY(t.transactedAt)
