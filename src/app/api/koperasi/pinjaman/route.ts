...
export async function POST(req: NextRequest) {
  ...
  if (anggotaData.pinjamans.some((p) => p.status === 'berjalan' || p.status === 'diajukan' || p.status === 'disetujui')) return NextResponse.json({ error: 'Masih memiliki pinjaman yang sedang berjalan' }, { status: 400 })
