...
CREATE INDEX idx_waste_item_category ON WasteItem(wasteCategoryId);
CREATE INDEX idx_saving_tx_user ON SavingTransaction(userId);
CREATE INDEX idx_saving_tx_qc ON SavingTransaction(qcStatus);
CREATE INDEX idx_sedekah_tx_user ON SedekahTransaction(userId);
CREATE INDEX idx_koperasi_anggota_status ON KoperasiAnggota(status);
CREATE INDEX idx_koperasi_pinjaman_status ON KoperasiPinjaman(status);
CREATE INDEX idx_product_active ON Product(isActive);
CREATE INDEX idx_product_online ON Product(dijualOnline);
CREATE INDEX idx_toko_order_status ON TokoOrder(orderStatus);
CREATE INDEX idx_article_published ON Article(publishedAt);
CREATE INDEX idx_kegiatan_published ON Kegiatan(publishedAt);
